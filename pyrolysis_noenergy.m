...
Mesh.a = (5e-3)^2; % surface area
Mesh.dv = Mesh.a * Mesh.dz; % volume 
...
% input mass 
m0(1,:) = (0.24092*0.0415659)/MW(1); % CELL 
m0(17,:) = (0.156317*0.0415659)/MW(17); % HCE
m0(24,:) = ((0.13672/3)*0.0415659)/MW(24); % LIGH
m0(25,:) = ((0.13672/3)*0.0415659)/MW(25); % LIGO
m0(23,:) = ((0.13672/3)*0.0415659)/MW(23); % LIGC
m0(38,:) = (0.0023269*0.0415659)/MW(38); % TGL
m0(37,:) = (0.003384*0.0415659)/MW(37); % TANN
m0(39,:) = (0.05676/0.02117)/MW(39); % moisture

mass0 = m0.*MW; % kg  inital mass of ea. cell in domain (mass matrix)
yi0 = mass0(s_index,1)./sum(mass0(s_index,1)); % normalize mass of ea. cell - div. by total mass (gives fraction)
rhos_mass0 = rhos_mass0+492; % 380 is density of particular sample kg/m3
sample_mass = 48.2817e-6; % Mesh.a*sample_height*rhos_mass0(1); % input exact mass from exp
mass0 = mass0./sum(mass0(s_index,1))*sample_mass./Mesh.Jnodes; % normalize ea. cell by mass of rep components / # nodes = mass of ea. component in ea. cell
...
Nu_qct = 4.36;
k_n2 = 0.05945; % [W/mK]
h_n2 = (Nu_qct*k_n2)/sample_height; % htc for marshalls exp [W/m2k]
...
for i = 1:nstep 
    ...
    ye(i+1,:) = yje; % ye contains mass fracs of components at top cell (heated surface cell) rows, ea. time step. columns are ea. gas phase species
    yy(i+1,:) = temp;
    yy1(i+1,:) = b(end,:);
...
%     h = 51; %h_n2; % 10: heat transfer coefficient for marshall xp
%     tr = 0;
%     
%     for i = 1:Mesh.Jnodes
%         yi(:,i) = m(s_index,i).*MW(s_index)./sum(m(s_index,i).*MW(s_index));
